...
class License
{
     ...
     * @var string
     */
    private $page_slug = 'betterlinks-settings';
    /**
     * Store URL
     * @var string
     */
    private $storeURL = BETTERLINKS_STORE_URL;
    /**
     * DEV MODE
     * @var bool
     */
    ...
    public function __construct($product_slug, $product_name, $text_domain)
    {
        ...
        $this->item_id      = BETTERLINKS_SL_ITEM_ID;
        /**
    ...
    public function init()
    {
        if (is_admin()) {
             ...
             * Activate License
             */
            add_action('wp_ajax_betterlinkspro/admin/activate_license', array( $this, 'activate_license' ));
            /**
             * Deactivate License
             */
            add_action('wp_ajax_betterlinkspro/admin/deactivate_license', array( $this, 'deactivate_license' ));
            /**
             * Admin Notices
             */
             ...
             * get license status
             */
            add_action('wp_ajax_betterlinkspro/admin/get_license', array($this, 'render_licenses_page'));
        }
    }
    /**
    ...
    public function activate_license()
    {
        ...
        $license_key 	= $this->get_hidden_license_key();
        $title 			= sprintf(__('%s License', $this->text_domain, 'betterlinks-pro'), $this->product_name);
        $status = $this->get_license_status();
    ...
    public function deactivate_license()
    {
        ...
        if ($request->license == 'deactivated') {
            $delete_status = delete_option($this->product_slug . '-license-status');
        ...
        wp_send_json_success(array('delete_status' => $delete_status, 'delete_license' => $delete_license));
        wp_die();
    }
    /**
        ...
        switch ($status) {
            case 'expired':
                $message = sprintf(
                    ...
                    '<a href="https://wpdeveloper.com/account">',
                    '</a>',
            ...
            switch (is_null($status) ? $response->error : $status) {
                ...
                case 'disabled':
                    $message = sprintf(
                        ...
                        "<strong>$this->product_name</strong>",
                        "<a href=". esc_url($this->support_url) .">",
                        "</a>"
                    );
                    break;
                case 'valid':
